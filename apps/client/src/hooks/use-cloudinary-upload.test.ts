...
describe("useCloudinaryUpload", () => {
  ...
  it("returns the CloudinaryUploadResult including public_id on success", async () => {
    ...
    // Held in an object: TS narrows a `let` assigned inside the callback to `never`.
    const uploaded: { value: CloudinaryUploadResult | null } = { value: null };
    await act(async () => {
      uploaded.value = await result.current.upload(blob);
    });

    expect(uploaded.value).toEqual(mockResult);
    expect(uploaded.value?.public_id).toBe("totaltidy/abc123");
